...
let
  ...
  adminUsers = lib.filterAttrs (_: user: user.policy.isAdministrator) cfg.users;
  sortedAdminNames = lib.sort (a: b: a < b) (builtins.attrNames adminUsers);
  firstAdminName = builtins.head sortedAdminNames;
...
in
{
  ...
  script = ''
    ...
    if [ "$STARTUP_WIZARD_COMPLETED" = "false" ]; then
      ...
      RESPONSE=$(${pkgs.curl}/bin/curl -s -X POST \
        -H "Content-Type: application/json" \
        -d '{"EnableRemoteAccess":${lib.boolToString cfg.network.enableRemoteAccess}}' \
        -w "\n%{http_code}" \
